...
**核心思路：** 在 Bing/Google 搜索 1688 工厂页面 → 提取店铺子域名 → 访问联系信息页面 → 获取电话、手机、地址和联系人 → 联系人缺失时 Kimi 优先调用天眼查补充 → 按多维度给工厂打分（信息完整度、工商可信度、公司规模、成立时长、1688 店龄、产品匹配度、物流距离、活跃度、合作伙伴、资质认证、企业官网、域名注册时间），并通过 Google/Bing 交叉验证降低风险。
...
3. **访问工厂页面** — 加载每个工厂黄页
4. **提取子域名和主营产品** — 通过无障碍树或文本匹配找到店铺链接（例如 `xxx.1688.com`），并提取主营产品用于评分和供应商数据库归档
...
9. **综合评分** — 从 12+ 维度给工厂打分（信息完整度、工商可信度、公司规模、成立时长、1688 店龄、产品匹配度、物流距离、活跃度、合作伙伴、资质认证、企业官网、域名注册时间），并通过 Google/Bing 交叉验证降低风险

## 输出格式

结果以 Markdown 表格形式返回：

| 公司 | 主营产品 | 电话 | 手机 | 地址 | 联系人 | 店铺 | 最近口岸 | 运输方式 | 物流分 |
|------|---------|------|------|------|--------|------|---------|---------|--------|
| 示例轴承有限公司 | 轴承、深沟球轴承 | 86 010 12345678 | 13800138000 | 北京市... | 张先生 | example.1688.com | 天津港 | 海运 | 8 |
...
- `references/factory-scoring.md` — 工厂综合评分与排序指南
